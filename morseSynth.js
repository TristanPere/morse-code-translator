...
function morseSynth() {
  ...
  this.morseCodes = {
    ...
    "<CL>": "−·−··−··", // CLosing down
    "<CT>": "−·−·−",
    "<KA>": "−·−·−", // Attention, Commencing Transmission
  ...
  (function (self) {
    ...
    self.oscillatorNode = self.context.createOscillator();
    ...
    for (var i = 0; i < character.length; i++) {
      ...
      if (character[i] == "·") {
        self.gainNode.gain.linearRampToValueAtTime(
          0,
          now + ((lengthSum + 1) * self.ditLength) / 1000
        );
        self.gainNode.gain.linearRampToValueAtTime(
          self.volume,
          now + ((lengthSum + 1) * self.ditLength) / 1000 + rampTime
        );

        self.gainNode.gain.linearRampToValueAtTime(
          self.volume,
          now + ((lengthSum + 2) * self.ditLength) / 1000
        );
        self.gainNode.gain.linearRampToValueAtTime(
          0,
          now + ((lengthSum + 2) * self.ditLength) / 1000 + rampTime
        );
        ...
        self.gainNode.gain.linearRampToValueAtTime(
          self.volume,
          now + ((lengthSum + 4) * self.ditLength) / 1000
        );
        self.gainNode.gain.linearRampToValueAtTime(
          0,
          now + ((lengthSum + 4) * self.ditLength) / 1000 + rampTime
        );
